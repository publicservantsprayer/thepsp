...
const USAStateContext = React.createContext(null)

export const UsaStateProvider: React.FC<Props> = ({ children, ...props }) => {
  const usaState = useUSAState(props)
  return (
...
export const useUSAStateContext = () => {
  const context = React.useContext(USAStateContext)
  if (!context) {
    throw new Error('useUSAStateContext must be used within a UsaStateProvider')
  }
  return context
}
